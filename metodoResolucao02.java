...
public class metodoResolucao {
    public static void main(String[] args) {
        try {
            Scanner in = new Scanner (new FileInputStream ("Entrada.in"));
            PrintStream out = new PrintStream (new FileOutputStream ("saida.out"));
            ...
            for (int k = 1; k < quantidade + 1; k++) {
                ...
                //A seguir eu retiro todos os espacos da proposicao atual
                ...
                out.println("Problema #" + k);
                if (funcoes.FNC(proposicao) == false) {
                    out.println("Não está na FNC.");
                } else {
                    if (funcoes.Horn(proposicao) == false) {
                        out.println("Nem todas as cláusulas são de Horn.");
                    } else { //Verificar se é satisfativel
                        if (funcoes.Satisfativel(proposicao) == false) {
                            out.println("Não, não é satisfatível.");
                        } else {
                            out.println("Sim, é satisfatível.");
                        }
                    }
                }
                out.println();
